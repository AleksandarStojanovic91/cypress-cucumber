Feature: Search for a job on Hello World and validate search results
  ...
  Scenario: Search for a Cypress based jobs on Hello World and validate the results
  ...
  Scenario: Search for a Selenium based jobs on Hello World and validate the results
    When I select 'QA' from menu
    And I search by IT technology 'Selenium'
    And I set 'intermediate' seniority
    And I sort by 'Najnovije'
    Then I verify search criteria 'QA' 'Selenium' 'Intermediate'
    When I open first search result
    Then I verify result content